...
public class EntityStrategy extends RouteStrategy {
	...
	public EntityStrategy(Entity entity, int accessBlockFlag) {
		boolean followed = false;//(entity.getAttributes().get("followed_by") != null);
		this.x = followed ? entity.getLastTile().getX() : entity.getTile().getX();
		this.y = followed ? entity.getLastTile().getY() : entity.getTile().getY();
