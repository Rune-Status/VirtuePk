...
public class WorldSubmissionDecoder implements PacketDecoder<WorldSubmissionHandler> {
	...
	@Override
	public WorldSubmissionHandler decodePacket(RS2PacketReader buffer, Session session, int opcode) {
		...
		for (int cycle = 0; cycle < size; cycle++) {
			...
			final int server = buffer.getInt();
			...
			//worlds.add(new WorldHub(id, name, server, flag, region, country, ip, location, playerCount, members, online));
		}
