...
import org.virtue.cache.util.CompressionUtils;
	...
	public static Container decode(ByteBuffer buffer) throws IOException {
		...
		if (type == COMPRESSION_NONE) {
			...
			if (type == COMPRESSION_BZIP2) {
				// System.out.println("Type=BZip2, decompressed size="+uncompressedLength+", compressedLength="+compressed.length);
				/*
				 * try (FileOutputStream out = new
				 * FileOutputStream("data/test_broken.bin")) {
				 * out.write(compressed, 0, compressed.length); }
				 */
				/*
				 * try (FileInputStream in = new
				 * FileInputStream("data/test_working.bin")) {
				 * in.read(compressed, 0, compressed.length); }//Problematic
				 * data: 145911 to 146151 (last 240 bytes)
				 * //System.out.println("Data: "+Arrays.toString(compressed));
				 * /*try { DataInputStream stream = new DataInputStream(new
				 * BZip2CompressorInputStream( new
				 * ByteArrayInputStream(compressed)));
				 * stream.readFully(uncompressed); stream.close(); } catch
				 * (IOException ioex) { ioex.printStackTrace(); System.exit(-1);
				 * return null; }
				 */
				...
				/*
				 * try { DataInputStream stream = new DataInputStream(new
				 * GZIPInputStream( new ByteArrayInputStream(compressed)));
				 * stream.readFully(uncompressed); stream.close(); } catch
				 * (IOException ioex) { ioex.printStackTrace(); return null; }
				 */
